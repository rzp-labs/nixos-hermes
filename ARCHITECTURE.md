...
This file is not a claim that every NixOS module has already moved to Den. Current implemented ownership is listed in `REVIEW.md`; as of this branch, hardware, Disko/ZFS layout, provisioning scripts, package overlay definitions, and runtime service modules remain native NixOS imports. SOPS/secrets, virtualization, users/Home Manager, host/system baseline, and install-time Disko path facts are rendered from Den.
